...
/*   ///////////////////////////////////////  Intersection part ///////////////////////////////////////////
void storeInArry(AVLTreeNode *node, int arry_1[], int arry_2[] ,int *index)
{
    if (node == NULL) return;
    storeInArry(node->left,arry_1,arry_2,index);
    arry_1[*index] = node->key;
    arry_2[*index] = node->value;
    (*index)++; //increase index for next pos
    storeInArry(node->right,arry_1,arry_2,index);
}

AVLTree *intersectionArry(int arry1_1[],int arry1_2[], int arry2_1[], int arry2_2[],int m, int n)
{
    int i = 0, j = 0;
    AVLTree *tree = newAVLTree();
    while (i < m && j < n)
    {
        if (arry1_1[i] < arry2_1[j])
            i++;
        else if (arry2_1[j] < arry1_1[i])
            j++;
        else if (arry1_1[i]== arry2_1[j]){
            if (arry1_2[i] < arry2_2[j])
                i++;
            else if (arry1_2[i] > arry2_2[j])
                j++;
            else
            {
                if (tree->size == 0)
                {
                    AVLTreeNode *newnode = newAVLTreeNode(arry1_1[i],arry1_2[i]);
                    tree->root = newnode;
                    newnode->height = 0;
                    tree->size++;
                }
                ...
                {
                    InsertNode(tree,arry1_1[i],arry1_2[i]);
                }
                j++;
                i++;
            }
...
AVLTree *AVLTreesIntersection(AVLTree *T1, AVLTree *T2)
{
    ...
    int *arr1_2[m];
    int i = 0;
    storeInArry(T1->root,arr1_1, arr1_2,&i);
    
    int n = T2->size;
    int *arr2_1[n];
    int *arr2_2[n];
    int j = 0;
    storeInArry(T2->root,arr2_1,arr2_2,&j);
    
    AVLTree *tree = intersectionArry(arr1_1, arr1_2,arr2_1, arr2_2,T1->size,T2->size);
    return tree;
}
*/
...
{ int i,j;
    ...
    ///tree7=AVLTreesIntersection(tree5, tree6); ////Not finish for this part,sorry...
