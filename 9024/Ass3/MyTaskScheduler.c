...
int TaskScheduler(char *f1, char *f2, int m )
{
    ...
    while (h1->size != 0 || h2->size != 0 )///还需要修改边界条件
    {
        int min_key = Min(h1);
        if (min_key <= currtime)
        {
        ...
        else if (min_key > currtime)
        {
            if (h2->size == 0) ////这里有些许问题
            {
                
                if (min_key >= MaxCore(core,m)){
                    for(i=0;i<m;i++)
                {
                    core[i] = min_key;
                }
                ...
                ///int core[m]= {currtime};/////这里还没有解决
                ///reset_time(core)///set all element in core equal to currunttime
                ///continue;
            }
            else
            {
                HeapNode *node_h2 = RemoveMin(h2);
                int i = MinCore(core,m);
                fprintf(fpWrite,"%d Core%d %d ",node_h2->TaskName, i+1, core[i]); ///Write file
                core[i] += node_h2->Etime;
                if (core[i] > node_h2->Dline)
                {
