...
HeapNode *RemoveMin(HeapNode *T)
{
    ...
    if (prey == NULL){
        T = T -> next; ///first node is min   ###改了这里
    }
    else {
        prey->next = y->next;
    }
...
HeapNode *HeapFind(HeapNode *node, int i)
{
    HeapNode *x =node;
    HeapNode *p = NULL;
    if(x->des == i)
...
void KeyDecrease(HeapNode *heap, HeapNode *node, int i)
{
    int temp;
    HeapNode *y = NULL, *z = NULL;
    node->key = i;
    y = node;
    z = y->parent;
    while (z != NULL && z->key > y->key)
    {
        SWAP(z->key, y->key, temp);
        SWAP(z->des,y->des,temp);
        y = z;
        z = y->parent;
    }
}
