...
class Peer:
	def __init__(self,id,pred,succ1,succ2,MSS,droprate):
		self.id = id
		self.port = PORTID + id
	...
	def message_helper(self, mtype, info1 = None, info2 = None, info3 = None, info4 = None):
			...
			try:
				if MESSAGETYPE[data[0]] == 'quit': # quit message received (update sucessors)
					...
					(self.pred > self.id and data[2] < self.id) :
						...
						message = self.message_helper(ENCR_MTYPE['fileres'],data[2], data[3]) ####file request model
						TCP_socket.send(message)
						print(f'A response message, destined for peer {data[1]}, has been sent.')
						TCP_socket.close()
						####传输文件
	...
	def file_function(self,filenum):
		...
		(self.pred > self.id and filehash < self.id):
			...
			try:
				...
				### 256 * date[3] + date[2]
				message = self.message_helper(ENCR_MTYPE['filereq'],filehash, filevalue)####file response model 'filereq'], filehash, filevalue, requester
				TCP_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
				TCP_socket.connect((HOST,PORTID+self.succ1))
				TCP_socket.send(message)
				TCP_socket.close()
